...
namespace Nop.Api.Framework.Validators
{
    public abstract class BaseNopValidator<T> : AbstractValidator<T> where T : class
    {
        ...
        protected virtual void SetDecimalMaxValue<TObject>(IDbContext dbContext)
        {
            ...
            foreach (var expr in expression)
            {
                RuleFor(expr.Value).IsDecimal(maxValues[expr.Key]).WithMessage(string.Format(localizationService.GetResource("Nop.Api.Framework.Validators.MaxDecimal"), maxValues[expr.Key] - 1));
            }
        }
    }
}